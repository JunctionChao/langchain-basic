...
if not milvus_client.has_collection(COLLECTION_NAME):
    ...
    # 4. 创建索引
    print("--> 正在为新集合创建索引...")
    sparse_index_params = {"index_type": "SPARSE_INVERTED_INDEX", "metric_type": "IP"} # IP: 内积相似度
    collection.create_index("sparse_vector", sparse_index_params)
    print("稀疏向量索引创建成功。")

    dense_index_params = {"index_type": "AUTOINDEX", "metric_type": "IP"}
...
if collection.is_empty:
    ...
    # 插入数据
    collection.insert([
        img_ids, paths, titles, descriptions, categories, locations,
        environments, sparse_vectors, dense_vectors
    ])
