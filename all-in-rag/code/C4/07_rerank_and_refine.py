import os
from typing import Sequence

import torch
import torch.nn.functional as F
from dotenv import load_dotenv
from langchain_classic.retrievers import ContextualCompressionRetriever
from langchain_classic.retrievers.document_compressors import (
    DocumentCompressorPipeline,
    LLMChainExtractor,
)

# 导入ColBERT重排器需要的模块
from langchain_classic.retrievers.document_compressors.base import (
    BaseDocumentCompressor,
)
from langchain_community.document_loaders import TextLoader
from langchain_community.vectorstores import FAISS
from langchain_core.documents import Document
from langchain_huggingface import HuggingFaceEmbeddings
from langchain_qwq import ChatQwen

from langchain_text_splitters import RecursiveCharacterTextSplitter
from transformers import AutoModel, AutoTokenizer

load_dotenv()


class ColBERTReranker(BaseDocumentCompressor):
    """ColBERT重排器"""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        model_name = "bert-base-uncased"

        # 加载模型和分词器
        # 调用最原始的 object 版本，不触发任何钩子、校验、缓存； 某些框架（如 Transformers、Pydantic）在初始化时快速写入字段，跳过用户自定义校验
        object.__setattr__(self, "tokenizer", AutoTokenizer.from_pretrained(model_name))
        object.__setattr__(self, "model", AutoModel.from_pretrained(model_name))
        # self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        # self.model = AutoModel.from_pretrained(model_name)
        self.model.eval()
        print("ColBERT模型加载完成")

    def encode_text(self, texts: list[str], max_len: int = 128) -> torch.Tensor:
        """ColBERT文本编码"""
        inputs = self.tokenizer(
            texts,
            return_tensors="pt",
            padding=True,
            truncation=True,
            max_length=max_len,
        )

        with torch.no_grad():
            outputs = self.model(**inputs)

        embeddings = outputs.last_hidden_state  # [B, L, d]
        embeddings = F.normalize(embeddings, p=2, dim=-1)  # 在最后一个维度上 L2 归一化

        # mask = inputs["attention_mask"].unsqueeze(-1)       # [B, L, 1]
        # embeddings = embeddings * mask                      # [B, L, d]  编码后，pad 位置的向量为还原0

        return embeddings

    def calculate_colbert_similarity(self, query_emb, doc_embs, query_mask, doc_masks):
        """ColBERT相似度计算（MaxSim操作）"""
        scores = []

        for i, doc_emb in enumerate(doc_embs):
            doc_mask = doc_masks[i : i + 1]

            # 计算相似度矩阵
            similarity_matrix = torch.matmul(
                query_emb, doc_emb.unsqueeze(0).transpose(-2, -1)
            )  # [1, L, d] * [1, d, L] = [1, L, L]

            # 应用文档mask
            doc_mask_expanded = doc_mask.unsqueeze(1)  # [1, 1, L]
            # 实际相似度值通常在[-1, 1]之间，总是大于-1e9
            similarity_matrix = similarity_matrix.masked_fill(
                ~doc_mask_expanded.bool(), -1e9
            )  # 文档mask 为 0 的位置，相似度设为负无穷

            # MaxSim操作
            max_sim_per_query_token = similarity_matrix.max(dim=-1)[0]  # [1, L]

            # 应用查询mask
            query_mask_expanded = query_mask.unsqueeze(0)  # [1, 1, L]
            # 相似度已经计算完成，查询填充位置的token没有实际语义，填充0不影响最终求和结果
            max_sim_per_query_token = max_sim_per_query_token.masked_fill(
                ~query_mask_expanded.bool(), 0
            )

            # 求和得到最终分数
            colbert_score = max_sim_per_query_token.sum(dim=-1).item()
            scores.append(colbert_score)

        return scores

    def compress_documents(
        self,
        documents: Sequence[Document],
        query: str,
        callbacks=None,
    ) -> Sequence[Document]:
        """对文档进行ColBERT重排序"""
        if len(documents) == 0:
            return documents

        # 编码查询
        query_inputs = self.tokenizer(
            [query], return_tensors="pt", padding=True, truncation=True, max_length=128
        )

        with torch.no_grad():
            query_outputs = self.model(**query_inputs)
            query_embeddings = F.normalize(query_outputs.last_hidden_state, p=2, dim=-1)

        # 编码文档
        doc_texts = [doc.page_content for doc in documents]
        doc_inputs = self.tokenizer(
            doc_texts,
            return_tensors="pt",
            padding=True,
            truncation=True,
            max_length=128,
        )

        with torch.no_grad():
            doc_outputs = self.model(**doc_inputs)
            doc_embeddings = F.normalize(doc_outputs.last_hidden_state, p=2, dim=-1)

        # 计算ColBERT相似度
        scores = self.calculate_colbert_similarity(
            query_embeddings,  # [B, L, d]
            doc_embeddings,  # [B, L, d]
            query_inputs["attention_mask"],  # [B, L]
            doc_inputs["attention_mask"],  # [B, L]
        )

        # 排序并返回前5个
        scored_docs = list(zip(documents, scores))
        scored_docs.sort(key=lambda x: x[1], reverse=True)
        reranked_docs = [doc for doc, _ in scored_docs[:5]]

        return reranked_docs


# 初始化配置
hf_bge_embeddings = HuggingFaceEmbeddings(model_name="BAAI/bge-large-zh-v1.5")

llm = ChatQwen(
    model="qwen-plus", temperature=0.1, api_key=os.getenv("DASHSCOPE_API_KEY")
)

# 1. 加载和处理文档
loader = TextLoader("../../data/C4/txt/ai.txt", encoding="utf-8")
documents = loader.load()
text_splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=100)
docs = text_splitter.split_documents(documents)

# 2. 创建向量存储和基础检索器
vectorstore = FAISS.from_documents(docs, hf_bge_embeddings)
base_retriever = vectorstore.as_retriever(search_kwargs={"k": 10})

# 3. 设置ColBERT重排序器
reranker = ColBERTReranker()

# 4. 设置LLM压缩器
compressor = LLMChainExtractor.from_llm(llm)

# 5. 使用DocumentCompressorPipeline组装压缩管道
# 流程: ColBERT重排 -> LLM压缩
pipeline_compressor = DocumentCompressorPipeline(transformers=[reranker, compressor])

# 6. 创建最终的压缩检索器
final_retriever = ContextualCompressionRetriever(
    base_compressor=pipeline_compressor, base_retriever=base_retriever
)

# 7. 执行查询并展示结果
query = "AI还有哪些缺陷需要克服？"
print(f"\n{'=' * 20} 开始执行查询 {'=' * 20}")
print(f"查询: {query}\n")

# 7.1 基础检索结果
print("--- (1) 基础检索结果 (Top 10) ---")
base_results = base_retriever.invoke(query)
for i, doc in enumerate(base_results):
    print(f"  [{i + 1}] {doc.page_content}\n")

# 7.2 使用管道压缩器的最终结果
print("\n--- (2) 管道压缩后结果 (ColBERT重排 + LLM压缩) ---")
final_results = final_retriever.invoke(query)
for i, doc in enumerate(final_results):
    print(f"  [{i + 1}] {doc.page_content}\n")
