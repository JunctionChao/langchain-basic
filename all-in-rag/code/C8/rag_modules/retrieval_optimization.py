...
class RetrievalOptimizationModule:
    ...
    def setup_retrievers(self):
        ...
        # 向量检索器    密集向量检索器, 能够捕捉到语义理解的准确性
        self.vector_retriever = self.vectorstore.as_retriever(
            search_type="similarity",
            search_kwargs={"k": 5}
        )

        # BM25检索器    基于 BM25 算法 的稀疏向量检索器, 保证关键词匹配的精确性
    ...
    def _rrf_rerank(self, vector_docs: List[Document], bm25_docs: List[Document], k: int = 60) -> List[Document]:
        ...
        for rank, doc in enumerate(vector_docs):
            ...
            logger.debug(f"密集向量检索 - 文档{rank+1}: RRF分数 = {rrf_score:.4f}")
        ...
        for rank, doc in enumerate(bm25_docs):
            ...
            logger.debug(f"BM25稀疏向量检索 - 文档{rank+1}: RRF分数 = {rrf_score:.4f}")
