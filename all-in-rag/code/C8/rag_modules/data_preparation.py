...
class DataPreparationModule:
    ...
    def load_documents(self) -> List[Document]:
        ...
        for md_file in data_path_obj.rglob("*.md"):
            try:
                ...
                try:
                    data_root = Path(self.data_path).resolve()  # 解析为绝对路径
    ...
    def _enhance_metadata(self, doc: Document):
        ...
        # 提取菜品名称
        doc.metadata['dish_name'] = file_path.stem  # 文件名（无扩展名）作为菜品名称
...
if __name__ == "__main__":
    # 测试数据准备模块
    data_pre = DataPreparationModule("../../../data/C8/cook")
    data_pre.load_documents()


"""
对于存储父子文档的ID映射关系parent_child_map后续没有使用，父文档和子文档的metadata中都保存了parent_id，
在后续寻找父文档时，通过遍历self.documents去匹配parent_id来寻找父文档。
可以优化为存储子文档 child_id和父文档的映射关系，映射关系中父文档直接指向self.document中的元素
"""